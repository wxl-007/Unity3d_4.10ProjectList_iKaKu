...
public class Water : MonoBehaviour
{
	...
	void LateUpdate()
	{
		...
		Vector3 offset = trans.position;
		Vector3 size = new Vector3(2000,600,2000);
		Vector4 os = new Vector4(offset.x, offset.z, 1.0f / size.x, 1.0f / size.z);
		mat.SetVector("_OffsetScale", os);
        //以下为后期更改.2013-09-30
        //change the position of the water, avoid of the camera view outside
        Vector3 pos = trans.position;
        pos.y = 0;
        if (pos != trans.position){
            transform.position = pos + new Vector3(0, 0, 500);
        }
	}
